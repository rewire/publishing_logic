shared_examples_for 'a model with publish logic' do
  def create_objects_with_different_published_at_dates(class_name)
    @object2 = make(class_name, :publishing_enabled => true, :published_at => 2.days.ago)
    @object1 = make(class_name, :publishing_enabled => true, :published_at => 1.days.ago)
    @object3 = make(class_name, :publishing_enabled => true, :published_at => 3.days.ago)
  end
  ...
  describe "ordering by published_at" do
    ...
    describe "by publication date oldest first" do
      ...
      it "should return the items, oldest first" do
        @class.by_publication_date_oldest_first.pluck(:id).should == [@object3,
                                                                     @object2,
                                                                     @object1].map(&:id)
      end

      it "should order by created_at date if published_ats are equal" do
        @object2b = make(@class, :publishing_enabled => true,
    ...
    it "should have a newest first ordering that is the reverse of the oldest first ordering for identical objects" do
      ...
      5.times { make(@class, :created_at => creation_time, :published_at => publish_time) }
